...
r_space = linspace(0.1,L2+sqrt(L3^2+L4^2),n_space); % x space
    ...
    for i_r = 1:n_space
        ...
        for i_y = 1:n_space
            ...
            for i_z = 1:n_space
                ...
                [T_ST,p_ST] = DefineWorkSpace(maxillaOn,mandibleOn,n_angle,ang_mouthOpen,T_SJ);
...
R_SJ = rot('z',pi/2);
p_SJ = Rvis*[r_cube;y_cube;z_cube];
T_SJ = [R_SJ p_SJ; zeros(1,3) 1];
[T_ST,p_ST] = DefineWorkSpace(maxillaOn,mandibleOn,n_angle,ang_mouthOpen,T_SJ);
...
posPRF = [r_space(i2)' y_space(i3)' z_space(i4)']';
posPRF = Rvis*posPRF;
figure(2)
...
%% save result
save('data/MecaBasePose_220913_maxilla','n_space','n_angle','L1','L2','L3','L4','L5',...
    'alpha','r_cube','y_cube','z_cube','alpha_space','r_space','y_space','z_space','posPRF','costNZ','p_PR','cost');