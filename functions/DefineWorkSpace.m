function [T_ST,p_ST] = DefineWorkSpace(maxillaOn,mandibleOn, occusalCutOn, axialCutOn, m,ang_mouthOpen,T_SJ)
...
elseif axialCutOn == 1
    % axial cut
    alpha = linspace(-80*pi/180,0,m);
    beta = linspace(-pi/6,pi/6,m);
    gamma = linspace(-5*pi/180,0,m);
end
...
for jj = 1:m
    R{jj} = R_JT*rot('z',alpha(jj));
    R{jj+m} = R_JT*rot('x',beta(jj));
