function RayFinder(startposition,Diameter,direction,RayLength)
...
%     CylinderLength = 1;
    ...
    h = repmat(direction/norm(direction)*RayLength,1,size(theta,2));
    points = repmat(startposition,1,size(theta,2))+Diameter/2*(v(:,1)*cos(theta)+v(:,2)*sin(theta))+h;
