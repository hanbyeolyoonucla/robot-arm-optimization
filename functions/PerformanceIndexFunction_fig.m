function performance = PerformanceIndexFunction_fig(X, mandibleOn, weight_m, weight_r, weight_s)
...
ISO_lin_max = max(ISO_lin,[],'all');
ISO_ang_max = max(ISO_ang,[],'all');
ISO_lin_std = std(ISO_lin,[],'all');
ISO_ang_std = std(ISO_ang,[],'all');
ISO_lin_mean = mean(ISO_lin,'all');
ISO_ang_mean = mean(ISO_ang,'all');
