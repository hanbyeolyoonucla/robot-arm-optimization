function performance = PerformanceIndexFunction_wTool(X)
% X = [L2, L3, L4, BETA, X_WS, Z_WS, X_FT, Z_FT, PHI_FT]
...
% 1. Input Varialbes
L1 = 0.064; L5 = 0.09; y_WS = 0; alpha = 0;
L2 = X(1); L3 = X(2); L4 = X(3);
beta = X(4);
x_WS = X(5); z_WS = X(6);
x_EH = X(7); z_EH = X(8); phi_EH = X(9);
...
for ii = 1:n_teeth
    for jj = 1:n_discrete  % AnalyticIK_wTool(n_joint,L1,L2,L3,L4,L5,x_FT,z_FT,phi_FT,S,M_EF,T_ST)
        [q_IK(:,ii,jj),eflag_IK,~] = AnalyticIK_wTool(n_joint,L1,L2,L3,L4,L5,x_EH,z_EH,phi_EH,S,M_EF,T_ST{ii,jj});
        % if no IK exits, break
        if eflag_IK == 0
            break
        else % if IK exists
            collisionStatus = RobotCheckCollision(checkCollisionOn,drawCylinderOn,S,q_IK(:,ii,jj),n_joint,7,...
                L1,L2,L3,L4,L5,Ltool,Ltool1,T_SJ);
